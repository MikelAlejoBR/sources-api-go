...
func TestEditSourceInvalidName(t *testing.T) {
	...
	emptyName := ""
	testValues := []*string{nil, &emptyName}
...
// TestEditSourceInvalidDuplicatedNameInTenant tests that the validation fails if the given source's name is not unique
// in the tenant. For this purpose it creates a new source in the database and then deletes it instead of using the
// existing fixture that is inserted in the main function. The reason is that it is easier to control this new fixture
// here than having to track the name of the previously inserted fixture, or exporting it to variable or whatever.
func TestEditSourceInvalidDuplicatedNameInTenant(t *testing.T) {
	testutils.SkipIfNotRunningIntegrationTests(t)
	...
	want := "name already exists in tenant"
	got := err.Error()

	if want != got {
		t.Errorf(`unexpected error when editing a source and giving it an already existing name. Want "%s", got "%s"`, want, got)
	}
